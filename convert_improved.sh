#!/bin/bash
# Script to convert text file to HTML with chapter navigation (improved)
...
# Parse the file and extract chapters - handle leading whitespace
awk '
BEGIN {
    chapter_num = 0
    in_chapter = 0
...
/第[0-9]+章/ {
    ...
    chapter_num++
    chapter_title = $0
    # Remove leading whitespace from chapter title
    gsub(/^[ \t　]+/, "", chapter_title)
    gsub(/"/, "\\\"", chapter_title)
    gsub(/`/, "\\`", chapter_title)
    print "        {"
...
echo "Total chapters found: $(grep -c '第[0-9].*章' "$INPUT_FILE")"
